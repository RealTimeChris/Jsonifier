...
// Use the provided definition.
	...
	struct exp_float_shuffle_table {
		...
		constexpr exp_float_shuffle_table() {
			for (int idx = 0; idx < 32 && enable; ++idx) {
				...
				for (int i = 0; i < 16; ++i)
					out[i] = 0x80;// shuffle high bit: output 0
	...
	JSONIFIER_INLINE auto to_bcd_4x4(int32x4_t efgh_abcd_mnop_ijkl, const data& d) noexcept -> uint8x16_t {
	...
	JSONIFIER_INLINE auto to_unshuffled_digits(uint64_t value, const data& d) -> uint8x16_t {
	...
	JSONIFIER_INLINE auto to_bcd_4x4(__m128i y, const data& d) noexcept -> __m128i {
	...
	template<int num_bits> JSONIFIER_INLINE auto to_digits(uint64_t value, const data& d) noexcept -> dec_digits<num_bits> {
	...
	template<> JSONIFIER_INLINE auto to_digits<32>(uint64_t value, [[ZMIJ_MAYBE_UNUSED]] const data& d) noexcept -> dec_digits<32> {
#if ZMIJ_USE_SSE4_1
	...
	JSONIFIER_INLINE void write_digits(char* buffer, dec_digits<64>::digits_type digits, bool drop_leading_zero, const data& d) noexcept {
	...
	JSONIFIER_INLINE void write_digits(char* buffer, uint64_t digits, bool drop_leading_zero, const data&) noexcept {
		memcpy(buffer, &digits, sizeof(digits));
		memmove(buffer, buffer + drop_leading_zero, sizeof(digits));
	}

	JSONIFIER_INLINE auto write_exp_float_simd(char* buffer, const dec_digits<32>& dig, int last_digit, bool has_last_digit, bool has_extra_digit, uint64_t exp_data,
		...
		auto entry		= d.exp_float_shuffles.get_entry(dig.num_digits, has_last_digit, has_extra_digit);
#if ZMIJ_USE_SSE4_1
		__m128i ascii	= _mm_or_si128(dig.unshuffled, _mm_load_si128(m128ptr(&d.zeros)));
	...
	JSONIFIER_INLINE auto write_exp_float_simd(char*, const dec_digits<64>&, int, bool, bool, uint64_t, const data&) noexcept -> char* {
		return nullptr;
	}
	...
	template<typename Float, typename UInt> JSONIFIER_INLINE auto to_decimal(UInt bin_sig, int64_t raw_exp, bool regular, const data& d) noexcept -> to_decimal_result {
		...
		template<typename Float> auto write(Float value, char* buffer) noexcept -> char* {
			...
			*buffer = '-';
			...
			if (traits::num_bits == 32 && exp_float_shuffle_table::enable) {
				...
				return write_exp_float_simd(buffer, dig, dec.last_digit, has_last_digit, has_extra_digit, exp_data, *d);
			}
