...
namespace jkj {
	namespace dragonbox {
		...
		template<class Format, class CarrierUInt, class ExponentInt = int> struct ieee754_binary_traits {
			...
			static constexpr carrier_uint mask = (carrier_uint(1) << total_bits) - 1u;
			...
			static constexpr bool is_positive(carrier_uint u) noexcept {
				return u < (carrier_uint(1) << total_bits);
			}
			...
			static constexpr auto significand_mask = ((((carrier_uint(1) << (Format::total_bits - 1)) - 1u) << 1) | 1u);
		...
		template<class Dummy> struct compressed_cache_holder<ieee754_binary32, Dummy> {
			...
			template<class ShiftAmountType, class DecimalExponentType> static JKJ_CONSTEXPR20 cache_entry_type get_cache(DecimalExponentType k) noexcept {
				...
				if (offset == 0) {
					...
					const auto pow5 = offset >= 7 ? detail::stdr::uint_fast32_t(detail::stdr::uint_fast32_t(pow5_table[6]) * pow5_table[offset - 6])
