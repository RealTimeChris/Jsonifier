...
namespace Jsonifier {
	...
	inline ValueIterator::ValueIterator(JsonIterator* jsonIter, uint32_t depth, uint32_t* start_position) noexcept
		: jsonIterator{ jsonIter }, currentDepth{ depth }, rootStructural{ start_position } {
	}
	...
	inline ErrorCode ValueIterator::end_container() noexcept {
		...
		// if (depth() <= 1 && !atEnd()) { return reportError(Incomplete_Array_Or_Object, "missing [ or { at start"); }
	...
	inline JsonifierResult<bool> ValueIterator::findFieldRaw(const std::string_view key) noexcept {
		...
		if (at_first_field()) {
			...
			if ((error = has_next_field().get(hasValue))) {
				abandon();
				return error;
			}
		}
		while (hasValue) {
			...
			// The call to skipChild is meant to skip over the Value corresponding to the key.
			// After skipChild(), we are right before the next comma (',') or the final brace ('}').
			JsonifierTry(skipChild());// Skip the Value entirely
   ...
   */
		...
		if (at_first) {
			...
			JsonifierTry(reset_object().get(hasValue));
			at_first = true;
			// 3. When a previous search found a Field or an iterator yielded a Value:
			//
			//    ```
			...
			// processed then skipChild() does not move the iterator (but may adjust the depth).
			if ((error = skipChild())) {
				abandon();
				return error;
			}
		...
		JsonifierTry(reset_object().get(hasValue));
		while (true) {
			assert(hasValue);// we should reach search_start before ever reaching the end of the Object
			assert(jsonIterator->currentDepth == currentDepth);// We must be at the start of a Field
			...
			// It returns true if ',' is found, false otherwise. If anything other than ',' or '}' is found,
			// then we are in error and we abort.
			error = has_next_field().get(hasValue);
			assert(!error);
