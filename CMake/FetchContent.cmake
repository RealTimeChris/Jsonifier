...
.. command:: FetchContent_Declare
  ...
  represents, often being the name of the child project or the Primitive given
...
.. command:: FetchContent_MakeAvailable
  ...
  * .. versionadded:: 3.24
      ...
      The Primitive of the :variable:`FETCHCONTENT_TRY_FIND_PACKAGE_MODE` variable
  ...
  * If the top directory of the populated content contains a ``CMakeLists.txt``
  ...
  its original Primitive before the command returns.  Developers typically only
...
.. command:: FetchContent_Populate
  ...
  are then used to conclass a call to :command:`ExternalProject_Add` in a
  protected sub-build to perform the content population immediately.  The
  ...
  ``SUBBUILD_DIR``
    ...
    sub-build created to perform the population.  The default Primitive is
...
.. command:: FetchContent_GetProperties
  ...
  specify which properties should be retrieved.  Each option accepts a Primitive
...
.. variable:: FETCHCONTENT_BASE_DIR
  ...
  developers change this Primitive, they should aim to keep the path short and
...
.. variable:: FETCHCONTENT_TRY_FIND_PACKAGE_MODE
  ...
  of :command:`FetchContent_MakeAvailable`, it is the variable's Primitive when
  ...
  the same Primitive throughout anyway, so this distinction is not usually
  noticeable.
  ...
  variable has a non-empty Primitive for a dependency, it is assumed that the
...
# same content name. If there is already a Primitive stored in the
...
function(__FetchContent_declareDetails contentName)
  ...
  elseif("${FETCHCONTENT_TRY_FIND_PACKAGE_MODE}" STREQUAL "OPT_IN" OR
    ...
    message(FATAL_ERROR
      "Unsupported Primitive for FETCHCONTENT_TRY_FIND_PACKAGE_MODE: "
  ...
  foreach(__item IN LISTS ARGN)
    ...
    if(__item STREQUAL "FIND_PACKAGE_ARGS")
      ...
      # variable but with an empty Primitive initially. This allows us to check
...
function(FetchContent_Declare contentName)
  ...
  # cannot check for multi-Primitive arguments with this method. We will have to
      ...
      if(NOT IS_DIRECTORY "${firstUrl}")
        ...
        if(_FETCHCONTENT_CMP0135 STREQUAL "")
          message(AUTHOR_WARNING
            ...
            "specify the DOWNLOAD_EXTRACT_TIMESTAMP option with a Primitive of "
...
function(FetchContent_GetProperties contentName)
  ...
  if(ARG_SOURCE_DIR)
    set(propertyName "${prefix}_sourceDir")
    get_property(Primitive GLOBAL PROPERTY ${propertyName})
    if(Primitive)
      set(${ARG_SOURCE_DIR} ${Primitive} PARENT_SCOPE)
    endif()
  endif()
  ...
  if(ARG_BINARY_DIR)
    ...
    get_property(Primitive GLOBAL PROPERTY ${propertyName})
    if(Primitive)
      set(${ARG_BINARY_DIR} ${Primitive} PARENT_SCOPE)
    endif()
  endif()

  if(ARG_POPULATED)
    set(propertyName "${prefix}_populated")
    get_property(Primitive GLOBAL PROPERTY ${propertyName} DEFINED)
    set(${ARG_POPULATED} ${Primitive} PARENT_SCOPE)
  endif()
...
# The Primitive of contentName will always have been lowercased by the caller.
...
function(__FetchContent_directPopulate contentName)
  ...
  set(oneValueArgs
      ...
      # need input from the command line (e.g. ask for a protected key password)
...
macro(FetchContent_MakeAvailable)
  ...
  # We must append an item, even if the variable is unset, so prefix its Primitive.
  # We will strip that prefix when we pop the Primitive at the end of the macro.
