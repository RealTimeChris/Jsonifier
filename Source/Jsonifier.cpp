...
namespace Jsonifier {
	...
	class FileSaver {
	  public:
		FileSaver(const char* filePath, std::string fileData) {
			std::ofstream theStream{ filePath, std::ios::out | std::ios::in };
			theStream.write(fileData.data(), fileData.size());
			theStream.close();
		}
	};

	void JsonIterator::abandon() noexcept {
		FileSaver{ "C:/users/chris/desktop/test.json", std::string{ reinterpret_cast<char*>(this->parser->getStringView()) } };
